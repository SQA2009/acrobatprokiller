...
class PDFViewer(QMainWindow):
    ...
    def queue_render_pages(self):
        """Queue rendering of all pages sequentially."""
        if not self.current_document:
            return

        with QMutexLocker(self.render_mutex):
            # Preenche a fila de renderização com todas as páginas, na ordem
            self.rendering_queue = list(range(len(self.page_widgets)))
    ...
    def handle_render_finished(self, page_number, pixmap):
        ...
        # Iniciar a próxima renderização
        self.process_next_render()
