    ...
    def paintGL(self):
        ...
        if self.image:
            ...
            if image_aspect > widget_aspect:
                ...
                scaled_width = 1.0
                scaled_height = image_aspect / widget_aspect
            else:
                # Ajustar com base na altura
                scaled_width = widget_aspect / image_aspect
                scaled_height = 1.0

            # Draw a textured quad ajustando as proporções
            glBegin(GL_QUADS)
            glTexCoord2f(0.0, 1.0)
            glVertex2f(-scaled_width, -scaled_height)
...
class PDFReader(QMainWindow):
    ...
    def display_page(self, page_index):
        ...
        # Converter o pixmap para um QImage
        image = QImage(
            pix.samples,
            pix.width,
            pix.height,
            pix.stride,  # Número de bytes por linha
            QImage.Format_RGB888,  # Formato correto para os dados do PyMuPDF
        )
        ...
        # Enviar imagem para o OpenGL para renderização
