...
class PDFViewer(QMainWindow):
    ...
    def render_page(self):
        ...
        try:
            ...
            # Cria QImage a partir dos dados renderizados
            img = QImage(pix.samples, pix.width, pix.height, pix.stride, QImage.Format_RGB888)

            # Converte QImage para um array NumPy
            img_array = np.asarray(img)
            if img_array.ndim < 3:  # Verifica se a conversão falhou
                raise ValueError("Erro na conversão da imagem para NumPy array.")
