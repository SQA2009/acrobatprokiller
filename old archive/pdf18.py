...
class PDFViewer(QMainWindow):
    ...
    def render_pages_incrementally(self):
        ...
        if self.current_page_rendering < len(self.current_document):
            # Renderiza uma página de cada vez
            page_number = self.current_page_rendering
            page_widget = self.create_page_widget(page_number)
            self.content_layout.addWidget(page_widget)
            self.current_page_rendering += 1

            # Continua renderizando a próxima página
            QTimer.singleShot(10, self.render_pages_incrementally)
        else:
    ...
    def create_page_widget(self, page_number):
        ...
        if (page_number, self.zoom_factor) in self.page_cache:
            pixmap = self.page_cache[(page_number, self.zoom_factor)]
        else:
    ...
    def update_zoom(self):
        ...
        self.current_page_rendering = 0  # Reinicia a renderização incremental
        self.render_pages_incrementally()
